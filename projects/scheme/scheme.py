...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(procedure,MacroProcedure):
            function= procedure.apply_macro(rest, env)
            return scheme_eval(function, env)
        else:
            with_env=lambda a: scheme_eval (a, env)
            args=rest.map(with_env)
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('Incorrect args.')
        # END PROBLEM 4
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        child = Frame(self.env).make_child_frame(self.formals, args)
        return child
        # END PROBLEM 12
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        first, second = expressions.first, expressions.second.first
        ...
        # BEGIN PROBLEM 10
        first, params, expr = expressions.first.first, expressions.first.second, expressions.second
        value= LambdaProcedure(params,expr,env)
        env.define(first,value)
        return first
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    parameters, values = nil, nil
    while bindings != nil:
        check_form(bindings.first, 2, 2)
        param, val = bindings.first.first, scheme_eval(bindings.first.second.first, env)
        parameters = Pair(param,parameters)
...
def do_define_macro(expressions, env):
    ...
    check_form(target, 1)
    res = MacroProcedure(target.second, expressions.second, env)
    env.define(target.first, res)
    return target.first
    # END Problem 21
