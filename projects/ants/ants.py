...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if isinstance(self.ant, QueenAnt) and self.ant.is_true_queen:
                return
            elif hasattr(self.ant, 'is_container') and self.ant.is_container and isinstance(self.ant.contained_ant, QueenAnt) and self.ant.contained_ant.is_true_queen:
                return
            # END Problem 13
...
class Ant(Insect):
    ...
    def __init__(self, armor=1, is_buffed=False):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        for i in range(self.min_range-1):
            check_place= check_place.entrance
        while not check_place is hive and place_number < self.max_range:
            if check_place.bees:
                return random_or_none(check_place.bees)
            else:
                check_place=check_place.entrance
                place_number+=1
        return None
        ...
        if not self.is_true_queen:
            ...
            while current_place is not None:
                if current_place.ant is not None:
                    if not current_place.ant.is_buffed:
                        current_place.ant.damage*=2
                        current_place.ant.is_buffed = True
                    if current_place.ant.is_container and current_place.ant.contained_ant is not None and not current_place.ant.contained_ant.is_buffed:
                        current_place.ant.contained_ant.damage *=2
                        current_place.ant.contained_ant.is_buffed = True
