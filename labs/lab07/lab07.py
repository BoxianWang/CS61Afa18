...
def store_digits(n):
    ...
    if n == 0:
        return Link.empty
    return Link(n // pow(10, len(str(n))-1), store_digits(n % pow(10, len(str(n))-1))) 
...
def is_bst(t):
    ...
    def bst_min(a):
        if a.is_leaf():
            return a.label
        return min(a.label, min([bst_min(b) for b in a.branches]))


    def bst_max(a):
        if a.is_leaf():
            return a.label
        return max(a.label, max([bst_max(b) for b in a.branches]))
